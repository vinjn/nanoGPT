...
nanogpt_folder = os.path.dirname(os.path.realpath(__file__))
exec(
    open(f"{nanogpt_folder}/configurator.py").read()
...
data_dir = os.path.join(nanogpt_folder, "data", dataset)
...
class Training:
    def __init__(self) -> None:
        # training loop
        self.X, self.Y = get_batch("train")  # fetch the very first batch
        self.t0 = time.time()
    ...
    def iterate(self):
        ...
        for micro_step in range(gradient_accumulation_steps):
            ...
            with ctx:
                logits, loss = model(self.X, self.Y)
                loss = (
                    loss / gradient_accumulation_steps
                )  # scale the loss to account for gradient accumulation
            # immediately async prefetch next batch while model is doing the forward pass on the GPU
            self.X, self.Y = get_batch("train")
        ...
        if iter_num % log_interval == 0 and master_process:
            ...
            print(
                f"iter {iter_num}: loss {lossf:.4f}, time {dt*1000:.2f}ms, mfu {self.running_mfu*100:.2f}%"
            )
        iter_num += 1
